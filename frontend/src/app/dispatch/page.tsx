...
export default function DispatchPage() {
  ...
  const handleDispatch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFeedbackMsg({
        type: "success",
        text: `✓ Mission [${res.mission_code}] successfully authorized and dispatched to ${
          res.target_location_name || res.target_location_id
        }.`,
      });
      setJustification("");
  ...
  return (
    <div
      className="flex-1 w-full p-6 sm:p-10 lg:p-14 space-y-8 relative overflow-hidden"
      ...
      >
        ...
        >
          <div className="space-y-2">
            ...
            <h1
              className="font-display-calm font-semibold tracking-tight"
              style={{
            ...
            <p className="type-body-sm" style={{ maxWidth: "55ch" }}>
              Priority-ranked tactical dispatch queue allocating scarce Urban SAR
              Battalions, Air Ambulances, and Field Hospitals where risk is
              highest.
            </p>
          </div>

          <div
            className="font-mono-data uppercase text-left md:text-right"
            style={{
          ...
          >
            Active Missions:{" "}
            <strong
              className="block mt-1 font-display-calm"
              style={{
                fontSize: "var(--text-xl)",
                color: "var(--status-intel-text)",
              }}
              aria-label="Active missions count"
            >
              <AnimatedCounter value={data?.active_missions_count || 0} />
            </strong>
          </div>
        </motion.div>
        ...
        {error && (
          <motion.div
            variants={itemVars}
            className="p-4 rounded-xl bg-rose-950/40 border border-rose-800 text-xs font-mono-data text-[#E11D48] flex items-center justify-between gap-2"
            role="alert"
          >
            <div className="flex items-center gap-2">
              <AlertTriangle className="w-4 h-4 flex-shrink-0" />
              <span>[DISPATCH_ERROR]: {error}</span>
            </div>
            <button
              onClick={loadData}
              type="button"
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 transition-colors cursor-pointer text-white"
            >
        ...
        {isLoading && !data && (
          <motion.div
            variants={itemVars}
            aria-label="Loading dispatch data…"
            className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-pulse"
          >
            ...
            <div className="lg:col-span-5 h-96 rounded-3xl bg-white/5 border border-white/10" />
          </motion.div>
        )}

        {/* Main Grid — only rendered when data is available */}
        {data && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-7 space-y-4">
              ...
              <motion.div variants={containerVars} className="space-y-4">
                {data.recommendations.length === 0 && (
                  <motion.div
                    variants={itemVars}
                    className="p-8 rounded-xl border border-white/10 bg-white/[0.02] text-center font-mono-data text-xs text-[#64748B] uppercase tracking-[0.15em]"
                  >
                    No sectors currently require priority dispatch.
                  </motion.div>
                )}

                {data.recommendations.map((rec, rIdx) => {
                  const isSelected =
                    selectedRec?.target_sector_id === rec.target_sector_id;
                  ...
                  return (
                    <motion.div
                      ...
                      aria-label={`Select sector ${rec.target_sector_name} for dispatch`}
                      onKeyDown={(e) => {
                        if (e.key === "Enter" || e.key === " ")
                          setSelectedSectorId(rec.target_sector_id);
                      }}
                      className="backdrop-blur-xl rounded-xl p-6 cursor-pointer transition-all"
                      style={{
                        background: "var(--bg-surface)",
                        border: isSelected
                          ? "1px solid var(--status-intel)"
                          : isTopPriority
                    ...
                    >
                      <div
                        className="flex flex-wrap items-center justify-between gap-2 pb-4 mb-4"
                      ...
                      >
                        ...
                        <div
                          className="font-mono-data uppercase flex flex-col items-end gap-1"
                          style={{
                            fontSize: "var(--text-2xs)",
                            letterSpacing: "var(--ls-wider)",
                            color: "var(--fg-tertiary)",
            ...
            <motion.div
              ...
              aria-label="Mission dispatch authorization console"
            >
              <div>
              ...
              {selectedRec ? (
                <form
                  ...
                  aria-label="Dispatch authorization form"
                >
                  <div>
                    <label className="block text-[#94A3B8] text-[10px] tracking-[0.2em] uppercase mb-3">
                      ...
                      {data?.resource_units
                        ...
                        .map((unit) => (
                          <div
                            ...
                            aria-label={`Select unit ${unit.unit_name}`}
                            onKeyDown={(e) => {
                              if (e.key === "Enter" || e.key === " ")
                          ...
                          >
                            <div className="flex items-center justify-between font-bold mb-1">
                              ...
                              <span className="text-[9px] text-[#34D399] tracking-[0.2em] uppercase bg-[#34D399]/10 px-2 py-1 rounded">
                                AVAILABLE
                              </span>
                            </div>
                            <div className="text-sm font-medium text-[#F3F4F6] mt-1 mb-2 font-display-calm tracking-wide">
                              {unit.unit_name}
                            </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      aria-label="Tactical justification for dispatch"
                  ...
                  <button
                    ...
                    aria-label="Authorize and dispatch mission"
                ...
                <div
                  className="py-24 text-center font-mono-data text-[10px] uppercase tracking-[0.2em] text-[#64748B]"
                  aria-label="Select a target sector to initialize dispatch"
                >
