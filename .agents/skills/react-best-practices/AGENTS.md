...
> generating, or refactoring React and frontend codebases. Humans  
...
Comprehensive performance optimization guide for React and frontend applications, designed for AI agents and LLMs. The guidance emphasizes portable rendering, async, bundle, and browser-performance patterns rather than framework-specific server features.
...
1. [Eliminating Waterfalls](#1-eliminating-waterfalls) — **CRITICAL**
   ...
   - 1.5 [Start Dependent Work As Early As Possible](#15-start-dependent-work-as-early-as-possible)
   - 1.6 [Strategic Suspense Boundaries](#16-strategic-suspense-boundaries)
2. [Bundle Size Optimization](#2-bundle-size-optimization) — **CRITICAL**
   - 2.1 [Avoid Barrel File Imports](#21-avoid-barrel-file-imports)
   - 2.2 [Conditional Module Loading](#22-conditional-module-loading)
   - 2.3 [Defer Non-Critical Third-Party Libraries](#23-defer-non-critical-third-party-libraries)
   - 2.4 [Lazy-Load Heavy Components](#24-lazy-load-heavy-components)
   - 2.5 [Preload Based on User Intent](#25-preload-based-on-user-intent)
3. [Server-Side Performance](#3-server-side-performance) — **HIGH**
   - 3.1 [Avoid Shared Module State for Request Data](#31-avoid-shared-module-state-for-request-data)
   - 3.2 [Cross-Request LRU Caching](#32-cross-request-lru-caching)
   - 3.3 [Hoist Static I/O to Module Level](#33-hoist-static-io-to-module-level)
   - 3.4 [Parallel Data Fetching with Component Composition](#34-parallel-data-fetching-with-component-composition)
...
4. [Client-Side Data Fetching](#4-client-side-data-fetching) — **MEDIUM-HIGH**
   ...
   - 4.2 [Use A Shared Client Cache For Request Deduplication](#42-use-a-shared-client-cache-for-request-deduplication)
...
6. [Rendering Performance](#6-rendering-performance) — **MEDIUM**
   ...
   - 6.7 [Use defer or async on Script Tags](#67-use-defer-or-async-on-script-tags)
   - 6.8 [Use Explicit Conditional Rendering](#68-use-explicit-conditional-rendering)
   - 6.9 [Use React DOM Resource Hints](#69-use-react-dom-resource-hints)
   - 6.10 [Use useTransition Over Manual Loading States](#610-use-usetransition-over-manual-loading-states)
...
### 1.5 Start Dependent Work As Early As Possible
...
### 3.1 Avoid Shared Module State for Request Data
...
export async function renderBadge() {
  ...
  return createBadge({ fontData, logoData });
}
```
...
export async function renderBadge() {
  ...
  return createBadge({ fontData: font, logoData: logo });
}
```
...
In long-lived server processes, module-level caching avoids repeated I/O across

requests. In serverless environments, each cold start still pays the initial

cost, but warm invocations benefit until the instance is recycled.
...
Use `React.cache()` for repeated non-fetch async work inside the same server

render, especially database queries, authentication checks, and expensive file
...
### 4.2 Use A Shared Client Cache For Request Deduplication
...
In SSR frameworks, some values are intentionally different on server vs client

(random IDs, dates, locale/timezone formatting). For these _expected_

mismatches, wrap the dynamic text in an element with

`suppressHydrationWarning` to prevent noisy warnings. Do not use this to hide

real bugs. Do not overuse it.
...
### 6.7 Use defer or async on Script Tags
...
### 6.10 Use useTransition Over Manual Loading States
...
2. [https://developer.mozilla.org](https://developer.mozilla.org)
3. [https://github.com/isaacs/node-lru-cache](https://github.com/isaacs/node-lru-cache)
4. [https://web.dev](https://web.dev)
